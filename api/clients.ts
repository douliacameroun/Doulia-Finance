...
export default async (req: VercelRequest, res: VercelResponse) => {
    ...
    if (req.method === 'GET') {
        try {
            ...
            res.status(200).json(Array.isArray(records) ? records.map(r => mapClient(r)) : []);
        } catch (error: any) {
        ...
        try {
            const body = req.body;
            const newRecord = await base(TABLES.CLIENTS).create([{ fields: { "Nom de l'Entreprise": body.name || body.nom,
